...
class _LawyerChatState extends State<LawyerChat> {
  ...
  Future<void> checkUserChats() async {
    var documentCheck = await FirebaseFirestore.instance.collection('LawyersChats').doc(FirebaseAuth.instance.currentUser!.email).get();
        ...
        fetchAllUserChats().then((data) {
          setState(() {
            for (int i = 1; i <= data['counter']; i++) {
              String dateTime;
              if(data['userID$i'][3] == DateTime.now().day && data['userID$i'][4] == DateTime.now().month) {
                String minute = data['userID$i'][1] < 10 ? '0${data['userID$i'][1]}' : '${data['userID$i'][1]}';
                String hour = data['userID$i'][2] >= 12 ? '${data['userID$i'][2] - 12}' : '${data['userID$i'][2]}';
                String amPm = data['userID$i'][2] >= 12 ? 'PM' : 'AM';
                dateTime = '$hour:$minute $amPm';
              } else {
                String date = data['userID$i'][3] < 10 ? '0${data['userID$i'][3]}' : '${data['userID$i'][3]}';
                dateTime = '${checkMonth(data['userID$i'][4])}, $date';
              }
              fetchLawyerData(data['userID$i'][0]).then((value) {
                setState(() {
                  chatList.add(Chats(lawyerID: value['id'], imageUrl: value['profilePic'], lawyerName: value['name'], time: dateTime, lastMsg: data['userID$i'][6], milliSeconds: data['userID$i'][5], messageBy: data['userID$i'][7]));
                });
              });
            }
            setState(() {
                ...
                child: InkWell(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => UserInbox(userId: itemData.lawyerID,lawyerId: FirebaseAuth.instance.currentUser!.email.toString(), imgUrl: itemData.imageUrl, name: itemData.lawyerName, isUser: false)));
                  },
                  child: Container(
                    width: 270,
                    height: 65,
                                ...
                                children: [
                                  ...
                                  itemData.messageBy == 'null'
                                    ...
                                    fontSize: 14,
                                      ...
                                      : Text('User: ${itemData.lastMsg}',style: const TextStyle(
                                    fontFamily: 'roboto',
                                    fontSize: 14,
                                  ),)
