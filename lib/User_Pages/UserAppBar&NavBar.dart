...
class _UserAppBarNavBarState extends State<UserAppBarNavBar> {
  ...
  Future<void> checkNotifications(Map<String, dynamic> userData) async {
    var documentCheck = await FirebaseFirestore.instance.collection('UsersNotifications').doc(userData['id']).get();
  ...
  Future<List<Map<String, dynamic>>> fetchAllLawyerData(String collectionName) async {
    List<Map<String, dynamic>> dataList = [];

    try {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection(collectionName).get();

      for (var doc in querySnapshot.docs.where((e) => e['isVerified'] == 'Verified')) {
              ...
              child: IconButton(
                  onPressed: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => UserNotificationPage(userData: userData,)));
                  },
                  icon: const Icon(
            ...
            child: Row(
              ...
              children: [
                SizedBox(
                  height: 55,
                  width: 50,
                  child: buildNavItem(FontAwesomeIcons.houseChimney, 'Home', 0),
                ),
                SizedBox(
                  height: 55,
                  width: 60,
                  child: buildNavItem(FontAwesomeIcons.rocketchat, 'Chats', 1),
                ),
                SizedBox(
                  height: 55,
                  width: 60,
                  child: buildNavItem(FontAwesomeIcons.solidHeart, 'Favourite', 2),
                ),
                SizedBox(
                  height: 55,
                  width: 60,
                  child: buildNavItem(FontAwesomeIcons.solidUser, 'Profile', 3),
                ),
              ],
            ),
          ),
...
class HomeScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return UserHomePage(dataList: dataList,userData: userData,);
  }
}
...
class FavoriteScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return UserFav(dataList: dataList, userData: userData,);
  }
}
