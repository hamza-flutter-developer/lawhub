...
class _UserNotificationPageState extends State<UserNotificationPage> {
  ...
  Future<void> checkNotification() async {
    var doc = await FirebaseFirestore.instance.collection('UsersNotifications').doc(widget.userData['id']).get();
    if(!doc.exists) {
      setState(() {
        isLoading = false;
  ...
  Future<void> updateNotificationSeenStatus(int index, String userId, String type, int counter) async {
    FirebaseFirestore.instance.collection('UsersNotifications').doc(widget.userData['id']).update({
      'Notification$index': [
        {'lawyerID': userId},
        {'type': type},
        {'isSeen': true},
